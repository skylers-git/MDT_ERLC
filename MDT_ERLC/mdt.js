...
// ── AUDIO CONTROLLER ──────────────────────────
// Single global object — prevents overlap/double-play and owns all state.
const audioController = {
  ambientVolume:   0.15,   // 15% default (per spec)
  fadeDuration:    2000,   // 2s fade-in for ambient
  muted:           false,
  unlocked:        false,  // true once user gesture has fired
  _fadeTimer:      null,
  _fadeInterval:   null,
  _greetingBound:  null,   // bound 'ended' handler ref for cleanup

  // ── Elements ──────────────────────────────────
  get greeting() { return document.getElementById('audio-greeting'); },
  get ambient()  { return document.getElementById('audio-ambient');  },

  // ── Phase 1: synthetic beep via Web Audio API ──
  beep() {
    try {
      ...
      osc.type      = 'sine';
      osc.frequency.setValueAtTime(1100, ctx.currentTime);       // high-pitched tone
      gain.gain.setValueAtTime(0.55, ctx.currentTime);
      gain.gain.exponentialRampToValueAtTime(0.001, ctx.currentTime + 0.10); // 100ms decay
      osc.start(ctx.currentTime);
      osc.stop(ctx.currentTime + 0.10);
      osc.onended = () => ctx.close();
    } catch(e) { /* Web Audio not available — skip silently */ }
  },

  // ── Fade ambient in from 0 → target over fadeDuration ──
  _fadeIn() {
    const a = this.ambient;
    if (!a) return;
    clearInterval(this._fadeInterval);
    clearTimeout(this._fadeTimer);
    const target   = this.muted ? 0 : this.ambientVolume;
    const steps    = 40;
    ...
    this._fadeInterval = setInterval(() => {
      ...
      a.volume = current;
      if (current >= target) clearInterval(this._fadeInterval);
    }, stepTime);
  },

  // ── Full sign-on sequence: beep → greeting → ambient (fade) ──
  playSignOnSequence() {
    // Guard: if already running, stop everything first
    this.stopAll();
    ...
    // Phase 1 — Beep (synchronous, ~100ms)
    this.beep();

    // Phase 2 — Greeting (slight delay so beep isn't clipped)
    if (greeting) {
      greeting.volume      = 1.0;
      greeting.currentTime = 0;

      // Remove any stale listener
      if (this._greetingBound) {
        greeting.removeEventListener('ended', this._greetingBound);
      }

      // Phase 3 — When greeting ends, fade ambient in
      this._greetingBound = () => {
        greeting.removeEventListener('ended', this._greetingBound);
        if (ambient) {
          ambient.play().then(() => this._fadeIn()).catch(() => {});
        }
      };
      greeting.addEventListener('ended', this._greetingBound);

      // Start greeting after beep (~120ms)
      setTimeout(() => {
        greeting.play().catch(() => {
          // Greeting missing/blocked — start ambient directly
          if (ambient) ambient.play().then(() => this._fadeIn()).catch(() => {});
        });
      }, 120);
    } else if (ambient) {
      // No greeting file — start ambient directly after beep
      setTimeout(() => {
        ambient.play().then(() => this._fadeIn()).catch(() => {});
      }, 250);
    }
  ...
  // ── Resume ambient only (session restore after refresh) ──
  resumeAmbient() {
    const ambient = this.ambient;
    if (!ambient) return;
    this.unlocked     = true;
  ...
  stopAll() {
    ...
    const a = this.ambient;
    if (g) {
      if (this._greetingBound) g.removeEventListener('ended', this._greetingBound);
      g.pause();
      g.currentTime = 0;
    }
  ...
  // ── Set volume from slider (0–100) ──
  setVolume(pct) {
    this.ambientVolume = pct / 100;
    const a = this.ambient;
    if (a && !this.muted) a.volume = this.ambientVolume;
    this.syncSlider();
    localStorage.setItem('mdt_audio_vol', pct);
  },
  ...
  toggleMute() {
    ...
    const a = this.ambient;
    if (a) a.volume = this.muted ? 0 : this.ambientVolume;
    localStorage.setItem('mdt_audio_muted', this.muted ? '1' : '0');
    this.syncButton();
  },

  // ── Sync the volume slider thumb position ──
  syncSlider() {
    const pctVal = Math.round(this.ambientVolume * 100);
    const slider = document.getElementById('audio-vol-slider');
    const label  = document.getElementById('vol-pct-display');
    if (slider) slider.value = pctVal;
    if (label)  label.textContent = pctVal + '%';
  },

  // ── Sync the mute/radio button icon ──
  syncButton() {
    ...
    if (!this.unlocked) {
      ...
      btn.title = 'Ambient radio — sign on to activate';
    } else if (this.muted) {
      btn.innerHTML = '🔇';
      btn.classList.add('muted');
      btn.title = 'Unmute ambient radio';
    } else {
      btn.innerHTML = '📻';
      btn.classList.remove('muted');
      btn.title = 'Mute ambient radio';
    }
  },

  // ── Load persisted prefs from localStorage ──
  loadPrefs() {
    const savedMute = localStorage.getItem('mdt_audio_muted');
    const savedVol  = localStorage.getItem('mdt_audio_vol');
    if (savedMute !== null) this.muted = savedMute === '1';
    if (savedVol  !== null) this.ambientVolume = parseInt(savedVol, 10) / 100;
    this.syncSlider();
    this.syncButton();
  }
};

// ── Public shims (keep existing call sites working) ──────────
function playLoginSequence()   { audioController.playSignOnSequence(); }
function resumeAmbientAudio()  { audioController.resumeAmbient(); }
function stopAllAudio()        { audioController.stopAll(); }
function toggleAudioMute()     { audioController.toggleMute(); }
function updateAudioButton()   { audioController.syncButton(); }
function onVolumeSlider(val) {
  audioController.setVolume(parseInt(val, 10));
  const pct = document.getElementById('vol-pct-display');
  if (pct) pct.textContent = val + '%';
}

function showResumeBanner() {
  const bar = document.getElementById('audio-resume-bar');
...
function signOn() {
  ...
  // Start audio — button click = trusted user gesture, safe from autoplay policy
  playLoginSequence();
}
...
(function() {
  ...
  // Restore audio prefs (mute + volume) from localStorage
  audioController.loadPrefs();
  ...
  if (savedName && savedCs) {
    ...
    // Session was active — we can't autoplay without a gesture, so show resume banner
    // (ambient audio requires a user interaction due to browser autoplay policy)
    showResumeBanner();
    audioController.syncButton();
  } else {
